...
UNS32 OnCOB_ID_SyncUpdate(CO_Data* d, UNS16 unsused_indextable, UNS8 unsused_bSubindex)
{
	...
	MSG_WAR(0x3001, "OnCOB_ID_SyncUpdate ", 0)
...
void startSYNC(CO_Data* d)
{
	...
	MSG_WAR(0x3001, "COB_ID_Sync", *d->COB_ID_Sync);
	MSG_WAR(0x3001, "Sync_Cycle_Period", US_TO_TIMEVAL(*d->Sync_Cycle_Period));
	if((*d->COB_ID_Sync & 0x40000000ul) && *d->Sync_Cycle_Period)
	{
		MSG_WAR(0x3001, "SetAlarm Sync_Cycle_Period", US_TO_TIMEVAL(*d->Sync_Cycle_Period));
